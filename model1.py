...
# 模拟系统
class Simulation:
    def __init__(self, num_builders=10, num_searchers=10, strategy_size_builders=5, strategy_size_searchers=10, num_strategies=20, decay_rate=0.5, probability=0.5, block_size=20, generations=500, temperature=10):
...
def plot_combined_heatmap(para_usage_count, rebate_usage_count, generations):  
    ...
    for param_index in range(2):  
        for start, end in ranges:  
            ...
            ax[param_index].set_xlabel('Generation')  
...
def plot_heatmap(para_usage_count, rebate_usage_count, generations):  
    ...
    for param_index in range(2):  
        for start, end in ranges:  
            ...
            ax.set_title(f'Strategy Fitness for Parameter $\\gamma_{{{param_index+1}}}$')  
            ax.set_xlabel('Generation')  
            ax.set_ylabel(f'Values of Parameter $\\gamma_{{{param_index+1}}}$')  
            ...
            ax.set_yticklabels(ytick_labels, rotation=0)  # Set rotation for y-tick labels to 0  
            ...
            plt.tight_layout()  
            plt.savefig(os.path.join('figures', f'fitness_evolution_heatmap_param_{param_index}.png'))  
            plt.show()  
            plt.close(fig)  
    ...
    for start, end in ranges:  
        ...
        ax.set_title(f'Strategy Fitness for Rebate Ratio $\\alpha$')  
        ax.set_xlabel('Generation')  
        ax.set_ylabel('Values of Rebate Ratio $\\alpha$')  
        ...
        plt.tight_layout()  
        plt.savefig(os.path.join('figures', 'fitness_evolution_heatmap_rebate.png'))  
        plt.show()  
        plt.close(fig)  
...
def plot_results_2(avg_rebate_history, avg_bid_history, generations):  
    ...
    plt.xlabel('Generation')  
    plt.ylabel('Average Rebate Ratio')  
    plt.legend(loc='lower right')  # 将图例固定在右下角  
    plt.xlim(0, generations)  # 设置 x 轴的范围  
    plt.tight_layout()  
    plt.savefig(os.path.join('figures', 'rebate.png'))  
    plt.show()  
    ...
    plt.xlim(0, generations)  # 设置 x 轴的范围  
    plt.tight_layout()  
    plt.savefig(os.path.join('figures', 'bid.png'))  
    plt.show()  
...
if __name__ == "__main__":
    generations = 10000
    sim = Simulation(num_builders=10, num_searchers=10, probability = 0.8, generations=generations, temperature=10)
    sim = sim.run()
    # sim.plot_results()  
